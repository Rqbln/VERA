...
export function RequirementDrawer({ row, runId, token, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/50" onClick={onClose}>
      <div
        className="h-full w-full max-w-lg overflow-y-auto border-l border-zinc-800 bg-zinc-950 p-4 text-xs"
      ...
      >
        <div className="mb-4 flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-zinc-500">{row.rationale}</p>
          </div>
          <button type="button" onClick={onClose} className="text-zinc-500 hover:text-zinc-300">
            ✕
          </button>
        </div>
        <dl className="mb-4 grid grid-cols-2 gap-2 text-zinc-400">
          <dt>Score</dt>
        ...
        <div className="mb-4">
          <div className="mb-1 text-zinc-500">Contributing benchmarks</div>
          <ul className="font-mono text-zinc-400">
            {row.contributing_benchmarks.map((b) => (
              <li key={b}>
                {b}
                {row.fallback_benchmarks.includes(b) ? (
                  <span className="ml-2 text-amber-500">fallback</span>
                ) : null}
              </li>
            ))}
          </ul>
        ...
        <div className="mb-4 flex gap-2">
          <button
            ...
            className="rounded border border-zinc-700 px-2 py-1 text-zinc-400 hover:bg-zinc-900"
          >
            benchmark_run.yaml
          </button>
          <button
        ...
        {raw?.rows?.length ? (
          <div>
            <div className="mb-1 text-zinc-500">Raw outputs (sample)</div>
            <pre className="max-h-64 overflow-auto rounded border border-zinc-800 bg-zinc-900 p-2 font-mono text-[10px] text-zinc-400">
