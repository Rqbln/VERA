...
% Inline TODO marker (the Overleaf draft pulled todonotes; this build stays lean).
\providecommand{\todo}[1]{\textcolor{red}{[TODO: #1]}}
% "Not aggregated" cell marker used in the TAM table.
...
In one review, a reviewer asked which tests had produced a score of \todo{v} on the requirement covering disclosure of artificial-intelligence use, and answering took \todo{n} minutes of reading a harness log of \todo{n} lines and \todo{n} model responses, even though every fact needed was already in the files.
\todo{Fill from the review record. If no single case can be reconstructed, give the typical time to answer one such question and say it is typical.}
The third is drift.
...
To close both gaps we built VERA, a self-hosted platform that carries an evaluation from launch to filed evidence with no specialist in the path, and put it into production in the responsible-AI function described above.
...
\begin{itemize}
  \item \textbf{VERA, an operable platform for EU AI Act evidence.} An open-source, self-hosted system that turns a completed evaluation into requirement verdicts a non-specialist can read, question and file, already used for release decisions in the deployment described above. Its design answers six requirements traced to regulatory text, to the tool literature and to a year of review practice.
  \item \textbf{Thirteen of the eighteen requirements measured.} VERA runs the twelve the reference taxonomy scores and instruments the energy of each evaluation run to add environmental impact as a thirteenth, leaving five on a documented human path rather than six.
  \item \textbf{Panel runs and a paired reading study.} Six open-weight models across three families and three sizes exercise all twelve measurable requirements under one configuration. In the reading study, correct answers rise from 18 to 32 of 48 and the median time per question falls from 56 to 29 seconds, with every per-participant value and the confounds that bound the comparison.
  \item \textbf{A replayable instrument.} The study runs as a page of the product, with consent, timing, validation and export built in, so any deployment can repeat the measurement on its own data.
\end{itemize}
...
At the requirement level, the Trust Factor is the weighted mean of the security-critical subset (robustness, cyber resilience, privacy, toxicity), renormalised over the requirements present in the run and scaled to 0--100, with default weights of $0.35$ for cyber resilience, $0.25$ for toxicity and $0.20$ each for robustness and privacy; the weights are overridable by configuration and recorded with the run.
On the refreshed run of \Cref{tab:panels}, $100 \cdot (0.20 \cdot 1.000 + 0.35 \cdot 0.375 + 0.20 \cdot 0.728 + 0.25 \cdot 0.750) = 66.4$, which is the stored value.
...
At commit \texttt{8a71cf3} (August 20, 2026) the suite holds 178 backend unit tests and 52 browser scenarios.
...
The five-requirement, fifteen-benchmark run of \Cref{tab:panels} against an 8B model reports $0.0169$\,kWh and $0.95$\,gCO$_2$e, measured by CodeCarbon 3.2.8 in online mode under the French grid-intensity factor.
These are the only numbers in the paper that come from direct measurement rather than from a benchmark score.
...
The generated corpus holds 230 documents, and both fallbacks reproduce on the refreshed run at one hundred and fifty items per benchmark.
...
\begin{table}[t]
  ...
  \begin{tabularx}{\columnwidth}{@{}l X@{}}
    ...
    Requirement set and version & mvp2-v2, digest \texttt{sha256:9e2b11d4} \\
    ...
    Baseline affordances & pretty-printed monospace text in scrollable panes, tab switching, browser text search; no line numbers, no download \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Nine completers reached it; five launched a run successfully at the first attempt, in a median of 55 seconds among successes, while one hit the five-minute cap, two gave up within seconds, and one pasted the address of the wrong page.
...
\Cref{tab:panels} reports a refreshed run of five requirements over four open-weight models of comparable size, at one hundred and fifty items per benchmark, computed by the same code for every model.
The five rows are chosen, not sampled: robustness, cyber resilience, privacy and toxicity are exactly the security-critical subset the Trust Factor aggregates, so the table lets a reader recompute the gauge end to end, and capabilities adds the one score most readers can anchor.
The environmental-impact figure of \Cref{sec:energy} accompanies each run, together with its wall-clock cost.

The rows that are cut from the full taxonomy are cut for stated reasons, not hidden.
The data-stage requirements CR03 and CR04 read the generated corpus rather than the model and are identical across models by construction; privacy CR05 is retained as the corpus-stage example because the Trust Factor needs it, and it is constant across models for the same reason.
Watermarking CR09 is zero for every open-weight model, a correct reading rather than a failed measurement.
Calibration CR07 is constant at $0.90$ because the value is a property of the confidence-binning procedure at this sampling budget rather than of the models, and a constant row adds no comparison.
Robustness R01 saturates at $1.00$ under its heuristic probe, which runs fallback-flagged because the serving path exposes no token log-probabilities; the flag appears in the table rather than being hidden.

The remaining rows separate the models.
Cyber resilience spans $0.250$ to $0.500$ with bootstrap intervals that no longer overlap the extremes, toxicity spans $0.596$ to $0.750$, and capabilities separates Mistral from the other three.
The added resolution matters: at ten items per benchmark an earlier draft reported Gemma's cyber resilience as $0.00$, and at one hundred and fifty items the same code reports $0.375$ with a confidence interval of $[0.355, 0.395]$ --- the zero was a resolution artefact, not a property of the model.
No model reaches the green band on the Trust Factor at this budget, and the four gauges sit within eight points of each other, which is the precondition for the reading study that follows: the interesting differences live below the aggregate, in the rows a reviewer has to open.

\begin{table*}[t]
  \caption{Five requirements over four open-weight models of comparable size, at seed 42 and 150 items per benchmark, on an Apple M4 Max (36\,GB). The five rows span the Trust Factor's security-critical set plus capabilities; \dag{} marks the fallback-flagged heuristic probe (no token log-probabilities on the serving path), \ddag{} the model-independent corpus scan. Brackets give bootstrap 95\% intervals where they are non-degenerate.}
  \label{tab:panels}
  \centering
  \tblsetup
  \begin{tabularx}{\textwidth}{@{}l l *{4}{>{\centering\arraybackslash}X}@{}}
    ...
    \midrule
    Robustness\,\dag & R01 & 1.000 & 1.000 & 1.000 & 1.000 \\
    Cyber resilience & R02 & 0.375 [.36, .40] & 0.250 & 0.375 [.36, .40] & 0.500 \\
    Privacy\,\ddag & R05 & 0.728 & 0.728 & 0.728 & 0.728 \\
    Capabilities & R06 & 0.800 & 0.800 & 0.800 & 0.700 [.69, .72] \\
    Toxicity & R12 & 0.750 & 0.625 & 0.750 & 0.596 \\
    \midrule
...
All four dropped out during or at the end of the baseline phase: two after its first question, two at the transition screen without seeing the dashboard phase, which biases the retained sample toward people who can read raw files and is therefore the conservative direction.
...
\begin{table}[t]
  ...
  \end{tabularx}
  \\[2pt]
  {\footnotesize No question reached the five-minute cap in either condition; every censored pair is an explicit give-up. P25 retains two of six time pairs, so its medians rest on two observations.}
\end{table}
...
The server timestamps revealed two such cases, both in the baseline phase; every other divergence is under ten seconds and consistent with tunnel transport latency.
...
The five-requirement, fifteen-benchmark run of \Cref{tab:panels} against an 8B model completes in 85 minutes on an Apple M4 Max with 36\,GB of memory, at 150 items per benchmark, which is short enough to re-run a model after a prompt change rather than only at a release gate.
