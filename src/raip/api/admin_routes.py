...
from raip.api.auth import ROLE_COMPLIANCE, AuthUser, require_roles
from raip.config import get_settings
from raip.governance.audit import recent_incidents
from raip.governance.bus import get_bus
from raip.governance.kill_switch import kill_switch_status, set_kill
from raip.governance.modes import VALID_MODES, all_modes, get_mode, set_mode
from raip.governance.trust_stream import current_trust, latest_signals, trust_series

router = APIRouter(prefix="/admin/v1", tags=["governance-admin"])

_GovUser = Annotated[AuthUser, Depends(require_roles(*ROLE_COMPLIANCE))]
_REGO_PATH = Path(__file__).resolve().parents[3] / "infra" / "opa" / "raip.rego"
...
def list_modes(_u: _GovUser) -> dict[str, Any]:
    return {"default": get_settings().raip_governance_mode, "models": all_modes()}
...
def put_policy(body: PolicyBody, _u: _GovUser) -> dict[str, Any]:
    ...
    try:
        url = f"{s.opa_url.rstrip('/')}/v1/policies/raip"
        resp = httpx.put(
