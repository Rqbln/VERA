...
Mirrors the :mod:`raip.artifacts.s3io` surface (``upload_bytes`` / ``download_bytes`` /
``presign_get``) so the platform runs without MinIO in lite mode. Artifacts are written under
``RAIP_LOCAL_ARTIFACTS_DIR`` and served back to the browser through the API route
...
def _root(settings: Settings) -> Path:
    root = Path(settings.raip_local_artifacts_dir).expanduser().resolve()
...
def presign_get(key: str, expires_in: int = 3600, settings: Settings | None = None) -> str | None:
    ...
    if not target.is_file():
        return None
    base = (s.raip_public_api_url or f"http://localhost:{s.api_port}").rstrip("/")
