...
record (reusing :func:`raip.governance.signing.sign_artifact`). Records are published to the
...
from raip.governance.bus import TOPIC_AUDIT, get_bus
...
def build_audit_event(
    *,
    ...
    block_below = float(os.environ.get("RAIP_POLICY_BLOCK_BELOW", "0.30"))
    incident = (
        decision == "deny"
...
def _jsonl_path(settings: Settings) -> Path:
    base = Path(settings.raip_local_artifacts_dir).expanduser().resolve() / "audit"
...
def index_opensearch(event: dict[str, Any], settings: Settings | None = None) -> bool:
    ...
    if not s.opensearch_url:
        return False
    try:
        url = f"{s.opensearch_url.rstrip('/')}/raip-audit/_doc/{event['event_id']}"
