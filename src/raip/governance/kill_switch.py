...
The flag is read from the environment (``RAIP_KILL_SWITCH``) and/or a Redis key, so it can be
...
_REDIS_KEY = "raip:kill_switch"
_REASON_KEY = "raip:kill_switch:reason"


def _env_killed() -> bool:
    return os.environ.get("RAIP_KILL_SWITCH", "").strip().lower() in ("1", "true", "yes", "on")
...
def kill_switch_status(settings: Settings | None = None) -> tuple[bool, str]:
    ...
    if _env_killed():
        return True, os.environ.get("RAIP_KILL_SWITCH_REASON", "env flag")
    try:
        r = _client(settings)
