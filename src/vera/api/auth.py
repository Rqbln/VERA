...
def auth_mode() -> str:
    ...
    return (os.environ.get("VERA_AUTH_MODE", "guided").strip().lower() or "guided")
...
def auth_disabled() -> bool:
    ...
    if auth_mode() == "guided":
        return True
    return os.environ.get("VERA_AUTH_DISABLED", "").lower() in ("1", "true", "yes")


def keycloak_issuer() -> str:
    base = os.environ.get("KEYCLOAK_URL", "http://localhost:8080").rstrip("/")
    realm = os.environ.get("KEYCLOAK_REALM", "vera")
...
def _dev_user() -> AuthUser:
    ...
    roles = os.environ.get("VERA_DEV_ROLES", default_roles).split(",")
    return AuthUser(
        sub="guided-user",
...
async def get_current_user(
    ...
    if not roles:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No VERA roles assigned")
