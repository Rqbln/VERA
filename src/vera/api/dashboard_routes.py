...
def _non_measurable_slots(rec: RunRecord) -> dict[str, Any]:
    ...
    from vera.schemas.declarative_forms import RedisFormStore
...
def health_stack() -> dict[str, Any]:
    ...
    try:
        from vera.artifacts.s3io import artifact_backend, ensure_bucket
        ...
        if artifact_backend(settings) == "local":
            checks["minio"] = {
                ...
                "required": False,
                "backend": "local",
                "dir": settings.vera_local_artifacts_dir,
            }
        else:
    ...
    try:
        ...
        checks["ollama"] = {
            ...
            "target_model": settings.vera_target_model,
        }
    except Exception as e:
...
def get_local_artifact(
    ...
    from vera.artifacts.local_fs import safe_path
...
def hitl_rubrics(_user: Annotated[AuthUser, Depends(get_current_user)]) -> dict[str, Any]:
    from vera.store.redis_hitl import RUBRICS
