...
from vera.benchmarks.runners.base import RawList, RunContext, SamplesByReq
from vera.benchmarks.runners.dataset_scan import run_dataset_scan
from vera.benchmarks.runners.fairness import run_fairness_r11
from vera.benchmarks.runners.garak_runner import run_garak
from vera.benchmarks.runners.hf_bbq import run_hf_bbq
...
from vera.benchmarks.runners.robustness import run_robustness_r01
from vera.benchmarks.runners.toxicity import run_toxicity_r12
from vera.benchmarks.runners.watermark import run_watermark, run_watermark_na
from vera.llm.client import LLMClient

# When VERA_REQUIRE_NATIVE=1, a heuristic fallback on a benchmark that has a native harness is a
# hard error (so a paper run is provably native) — except implementations listed here, which may
# legitimately fall back (e.g. garak does not run on Apple Silicon). Override via VERA_NATIVE_ALLOW.
_DEFAULT_NATIVE_ALLOW = {"garak"}


def _require_native() -> bool:
    return os.environ.get("VERA_REQUIRE_NATIVE", "").strip().lower() in ("1", "true", "yes")


def _native_allow() -> set[str]:
    extra = {x.strip() for x in os.environ.get("VERA_NATIVE_ALLOW", "").split(",") if x.strip()}
    return _DEFAULT_NATIVE_ALLOW | extra


class NativeHarnessRequired(RuntimeError):
    """Raised when VERA_REQUIRE_NATIVE is set and a benchmark fell back to a heuristic."""
        ...
        if _require_native() and impl not in _native_allow():
            ...
            if fell_back:
                raise NativeHarnessRequired(
                    ...
                    f"({fell_back[0]}) but VERA_REQUIRE_NATIVE is set"
                )
