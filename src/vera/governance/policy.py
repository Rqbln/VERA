...
def _thresholds() -> tuple[float, float]:
    block = float(os.environ.get("VERA_POLICY_BLOCK_BELOW", "0.30"))
    warn = float(os.environ.get("VERA_POLICY_WARN_BELOW", "0.60"))
    return block, warn


def builtin_decision(inp: dict[str, Any]) -> dict[str, Any]:
    """Reference policy, mirrored in infra/opa/vera.rego."""
...
def evaluate_policy(inp: dict[str, Any], settings: Settings | None = None) -> dict[str, Any]:
    ...
    if s.opa_url:
        try:
            url = f"{s.opa_url.rstrip('/')}/v1/data/vera/governance/decision"
