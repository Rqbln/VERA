...
from vera.governance.trust_stream import current_trust
...
def _forward(body: dict[str, Any], settings: Settings) -> tuple[str, dict[str, Any], float]:
    ...
    model = str(body.get("model") or settings.vera_target_model)
    t0 = time.monotonic()
    res = client.completion(
        ...
        api_base=settings.proxy_target,  # honour VERA_PROXY_TARGET_URL
    )
...
def govern(body: dict[str, Any], settings: Settings | None = None) -> tuple[int, dict[str, Any]]:
    ...
    model = str(body.get("model") or s.vera_target_model)
    ...
    if decision["decision"] == "deny" and mode == "enforcement":
        ...
        return 503, {"error": "blocked by governance policy", "vera_governance": governance}
